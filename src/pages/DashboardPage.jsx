...
export default function DashboardPage() {
  ...
  const cardGrid = resp.isMobile ? '1fr' : resp.isTablet ? '1fr 1fr' : '1fr 320px';

  return (
    <>
      <SEO title="Dashboard" url="/dashboard" noindex />
      <div style={{ width: '100%' }}>
        <div style={{ background: `linear-gradient(135deg, ${C.orange}22 0%, ${C.orange}08 60%, transparent 100%)`, border: `1px solid ${C.orange}30`, borderRadius: 20, padding: resp.isMobile ? '16px' : '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 12, color: C.orange, fontWeight: 700, marginBottom: 4, letterSpacing: '0.05em' }}>{greet.toUpperCase()} 👋</div>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: resp.isMobile ? 18 : 22, color: T.text, marginBottom: 4 }}>{user?.name?.split(' ')[0] || 'Pengguna'}!</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fill, minmax(${resp.isMobile ? '140px' : '200px'}, 1fr))`, gap: 16, marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: cardGrid, gap: 20, marginBottom: 24 }}>
          <Card style={{ padding: 0 }}>
            <div style={{ padding: resp.isMobile ? '14px 16px 12px' : '18px 20px 14px', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ padding: '10px 0' }}>
              {programList.length > 0 ? programList.map(p => (
                <div key={p.id} style={{ padding: resp.isMobile ? '12px 16px' : '14px 20px', borderBottom: `1px solid ${T.border}`, cursor: 'pointer' }} onClick={() => navigate('/belajar')}>
          ...
          <Card style={{ padding: 0 }}>
            <div style={{ padding: resp.isMobile ? '14px 16px 12px' : '18px 20px 14px', borderBottom: `1px solid ${T.border}` }}>
            ...
            <div style={{ padding: '10px 0' }}>
              {upcomingSchedule.filter(ev => ev.type !== 'none').map(ev => (
                <div key={ev.id} style={{ padding: resp.isMobile ? '12px 16px' : '14px 20px', borderBottom: `1px solid ${T.border}`, display: 'flex', gap: 12, cursor: 'pointer' }} onClick={() => navigate(ev.type === 'live' ? '/live' : '/tryout')}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: cardGrid, gap: 20 }}>
          <Card style={{ padding: 0 }}>
            <div style={{ padding: resp.isMobile ? '14px 16px 12px' : '18px 20px 12px', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
            ...
            <div style={{ padding: '16px 8px', height: resp.isMobile ? 180 : 220 }}>
